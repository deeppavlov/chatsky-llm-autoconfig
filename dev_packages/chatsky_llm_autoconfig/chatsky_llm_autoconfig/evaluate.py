...
def generate_graph(dialogue, model_name):
    return json.loads(dialog_model.create_graph(
        dialogue,
        model=ChatOpenAI(model=model_name, api_key=os.getenv("OPENAI_API_KEY"), base_url=os.getenv("OPENAI_BASE_URL"), temperature=0)
    ))
...
def calculate_mean_metrics(all_metrics):
    ...
    return {
        "Mean Triplet Match Accuracy": mean_triplet_match,
        "Mean Node Accuracy": mean_node_accuracy,
        "Mean Edge Accuracy": mean_edge_accuracy
    }
...
def evaluate_model(input_json_path, output_directory, model_name):
    ...
    for idx, dialogue in enumerate(dialogues):
        ...
        save_graph_comparison(
            target_graph,
            generated_graph,
            f'{output_directory}/graph_comparison_{idx}.png'
        )
