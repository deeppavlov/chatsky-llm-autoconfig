...
class DialogueSampler(DialogueGenerator):
    ...
    def invoke(self, graph: BaseGraph, start_node: int = 1, end_node: int = -1, topic="") -> list[Dialogue]:
        ...
        all_dialogues = []
        start_nodes = [n for n, attr in nx_graph.nodes(
            data=True) if attr.get("is_start", n == start_node)]
        ...
        for start in start_nodes:
            ...
            while stack:
                ...
                current_utterance = random.choice(
                    nx_graph.nodes[current_node]["utterances"])
                path.append({"text": current_utterance,
                ...
                if current_node == end_node:
                    ...
                    if edges:
                        ...
                        path.append(
                            {"text": last_edge_utterance, "participant": "user"})
                ...
                for source, target, edge_data in edges:
                    ...
                    edge_utterance = random.choice(edge_data["utterances"]) if isinstance(
                        edge_data["utterances"], list) else edge_data["utterances"]
